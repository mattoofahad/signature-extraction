...
def extract_signature(img, clf, preprocess=True):
    ...
    Parameters:
        ...
        preprocess : whether to apply additional preprocessing to the image
    ...
    for (descriptors, idx) in components:
        ...
        if 0.5 < confidence < 0.99:
            mask[idx] = 255

    return mask


def extract_signatures(dataset, out_dir, model, preprocess=True, use_color=False):
    ...
    Parameters:
        ...
        out_dir (str) : Path to save the segmentation output.
        ...
        use_color (bool) : Optional. Whether to use color images. Default is
                          False.
    """
    ...
    if not os.path.exists(out_dir + "/masks/"):
        os.makedirs(out_dir + "/masks/")
    ...
    for image_f in tqdm(images):
        ...
        outfile = os.path.split(image_f)[1]
        outfile = os.path.splitext(outfile)[0] + ".png"
        outfile = os.path.join(out_dir + "/masks/", outfile)
